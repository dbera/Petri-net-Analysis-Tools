...
class Walker 
{
	...
	public void generateRG(NetState net_state, int level_index)
	{
		System.out.println(">>>>>>>>>>>");
		...
		try { current_state_name = generateCurrentStateLabel(n.getCurrentStateName()); } 
		...
		if(level_index > 10  || isVisitedState) {
			if(level_index > 10) is_state_space_generated = false;
			if(isVisitedState) {
		...
		while(enbList.size() > 0)
		{
			...
			// Recursion
			generateRG(n.getCurrentState(), level_index+1);
