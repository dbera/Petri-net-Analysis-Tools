...
{	
	...
	// added to record state name -> marking
	private static Map<String, String> mapNodeIDToStateName; // to be updated at begining of each recursion! Node ID is unique
	...
	public static GraphLogger getInstance() {
		if (graphLoggerInst == null) {
			...
			edgeIdx = 0;
			mapNodeIDToStateName = new HashMap<String, String>();
		}
	...
	// updating of state names to node ID
	public void addToMapNodeIDToStateName(String nodeId, String stateName) { mapNodeIDToStateName.put(nodeId, stateName); }
	public String getStateName(String nodeId) { return mapNodeIDToStateName.get(nodeId); }
	...
	public void writeToBrScnFormat()
	{
		...
		for(String elm : visitedNodes) writer.println("an " + elm + " label: \"" + elm + "\""  + " ui.name =  \"" + getStateName(elm) + "\" \n");
